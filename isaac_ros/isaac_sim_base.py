...
class IsaacBase():
    ...
    def spawn_robots(self, config_path : str):
        # Only wheeled robots are supported right now
        with open(config_path, "r") as file:
            config_file = yaml.safe_load(file).get("robots")
            for r_conf in config_file.values():
                # robot_path = self.__get_assets_root_path() + r_conf.get("isaac_asset_path")
